...
module.exports = class Server {
	...
	setup() {
		...
		this.rcon('sv_rcon_whitelist_address ' + this.cfg.nconf.get('ip') + ';logaddress_add ' + this.cfg.nconf.get('ip') + ':' + this.cfg.nconf.get('port') + ';log on');
		this.status();

		const that = this;
		setTimeout(function () {
			that.chat(' \x10Hi! I\'m OrangeBot.' + (that.state.admins.length > 0 ? ' \x0e' + that.state.admins.join(', ') + '\x10 is now my admin.' : ''));
	...
	clantag(team) {
		...
		for (const i in this.state.players) {
			if (this.state.players.hasOwnProperty(i) && this
					.state.players[i].team === team && this.state.players[i].clantag !== undefined) {
